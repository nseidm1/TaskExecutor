...
public class TaskExecutorService extends Service implements ServiceExecutorCallback {
    ...
    /**
     * @param SERVICE_MODE
     *            Provide a Service mode, either CALLBACK_INCONSIDERATE, or
     ...
     * @param AUTOEXEC_MODE
     *            Provide an auto execute mode, either AUTOEXEC_MODE_ENABLED, or
     *            AUTOEXEC_MODE_DISABLED. The service will auto execute queued
     *            Tasks every five seconds automatically without needing to
     *            manually call {@link TaskExecutor#executeQueue()}.
     * @param context
     ...
     *            restored by the service after a restart. This interface is
     *            ONLY called when the service is in CALLBACK_DEPENDENT mode.
     */
    public static void requestTaskExecutorReference(int SERVICE_MODE, int AUTOEXEC_MODE, Context context, ExecutorReferenceCallback executorReferenceCallback, TasksRestoredCallback tasksRestoredCallback) {
	...
	// Only clear the callbacks if the number of reference requests is 0.
	// This will prevent a race condition if multi reference requests

...
/**
 * 
 * Welcome to the Task. This is where you get stuff done. There's one abstract
 * method just like a Runnable that you'll implement to get your work done. The
 * Task has the two main callbacks you're accustomed to in AsyncTasks,
 * {@link #postUpdate(Bundle)}, and {@link #postComplete(Boolean, Exception)}.
 * postComplete is automatically managed, but set to protected if you want to
 * define it yourself. By default it will pass the main Task bundle to the
 * TaskExecutor's onTaskComplete method that you end up defining in your
 * Activity's abstract implementation. Updates don't have a default
 * implementation so you'll want to use postUpdate when you define the
 * {@link #task()} method; postUpdate will execute on the UI thread.
 */
public abstract class Task implements Runnable {
    ...
    /**
     * Define your desired behavior if no activity is available when your Task
     * completes execution. Basically this defines how
     * {@link #postComplete(Boolean, Exception)} handles the main Task bundle.
     * 
     * @return Boolean
     */
    ...
    /**
     * Use this method to post updates to the ui thread. Pass strategic bundles
     * that the callback is defined to process. Use this method strategically
     * when you define the abstract Task method.
     */
     ...
     * @param shouldRemove
     * @param exception
     *            The main bundle of the Task will be passed to the
     *            TaskCompletedCallback managed by the TaskExecutor, or it will
     *            be queued for delivery if no currently visible Activity is
     *            available. You can enable and disable the queueing
     *            {@link #queueResultsIfNoActivity()}
     */
    private void postComplete(final boolean shouldRemove, final Exception exception) {
	if (shouldRemove)
	...
	public PersistenceObject(String className, Bundle bundle, String TAG, boolean removeFromQueueOnSuccess, boolean removeFromQueueOnException) {
